...
class RollingOver3:
  ...
  fatigue=0
  max_prone_orientation_reward=0
  max_quat_dist=0
  ...
  def step(self,action):
    ...
    self.max_quat_dist=np.maximum(quat_dist,self.max_quat_dist)
    prone_orientation_reward=self.parametric_gaussian(quat_dist,mean=0,sigma=1)

    self.max_prone_orientation_reward=np.maximum(prone_orientation_reward,self.max_prone_orientation_reward)
    reward+=prone_orientation_reward

    self.fatigue+=self.fatigue_weight*np.sum(np.square(action))
  ...
  def _check_terminal_conditions(self,joint_pos,chest_accel,torso_accel):
    ...
    """
    if self.fatigue>self.energy_reserves:
      return True
    return False
  ...
  def reset(self):
    print(int(self.fatigue),self.step_number)
    print("Max Prone Orientation Reward:",self.max_prone_orientation_reward)
    self.log("rollout/quat_dist",self.max_quat_dist)
    self.step_number=0
    self.fatigue=0
    self.max_prone_orientation_reward=0
    self.max_quat_dist=0
