...
class GettingUp2:
  ...
  7. **Control Cost Reward**: Penalizes the magnitude of the control inputs to encourage smoother movements.
  ...
  MIN_UPRIGHT_HEIGHT = 0.6  # Minimum height when upright
  MAX_HEAD_TILT = deg2rad(60)  # Maximum head tilt allowed
  STABLE_STEPS_REQUIRED = 30  # Number of steps required to consider position stable
  ...
  def init(self,total_timesteps=0):
    ...
    self.stable_upright_steps = 0
    self.max_achieved_height = 0.0
  ...
  def step(self,action):
    ...
    reward=base_reward+orientation_penalty+acceleration_penalty+position_penalty

    ctrl_cost=self._ctrl_cost_weight*np.sum(np.square(action))
    reward-=ctrl_cost

    # Apply Gaussian reward for each action element
    action_reward=np.sum(self.parametric_gaussian(action,mean=0,sigma=0.25)
  ...
  def _check_terminal_conditions(self, joint_pos, chest_accel, torso_accel):
    """
    Enhanced terminal conditions checker that combines both immediate failure conditions
    and success conditions for getting up.
    """
    qpos = self.env.data.qpos
    quat = qpos[3:7]
    pos = qpos[0:3]

    # Get euler angles using transforms3d
    euler = transforms3d.euler.quat2euler(quat, 'sxyz')
    roll = abs(euler[0])
    pitch = abs(euler[1])

    # Update maximum achieved height
    current_height = pos[2]
    self.max_achieved_height = max(self.max_achieved_height, current_height)
    ...
    # Check if dropped too low after achieving some height
    height_threshold = self.max_achieved_height - 0.3
    if current_height < height_threshold and self.max_achieved_height > self.MIN_UPRIGHT_HEIGHT:
      return True

    # Check acceleration limits (indicates instability or falling)
    if (np.linalg.norm(chest_accel) > self.MAX_ACCEL or
        np.linalg.norm(torso_accel) > self.MAX_ACCEL):
      return True

    # 2. Time-based conditions

    # Check if robot is upright and stable
    is_upright = roll < 0.3 and pitch < 0.3
    gravity_vector = np.array([0, 0, -9.81])

    # Calculate alignment with gravity using accelerometer data
    chest_alignment = np.dot(chest_accel, gravity_vector) / (np.linalg.norm(chest_accel) * np.linalg.norm(gravity_vector))
    torso_alignment = np.dot(torso_accel, gravity_vector) / (np.linalg.norm(torso_accel) * np.linalg.norm(gravity_vector))

    # Update stable steps counter
    if is_upright and chest_alignment < -0.9 and torso_alignment < -0.9 and current_height > self.MIN_UPRIGHT_HEIGHT:
      self.stable_upright_steps += 1
    else:
      self.stable_upright_steps = 0

    # Terminate if taking too long to get up
    if self.step_number > 200 and not is_upright:
      return True

    # Success condition - stable and upright for sufficient time
    if self.stable_upright_steps >= self.STABLE_STEPS_REQUIRED:
      return True
  ...
  def reset(self):
    ...
    qpos[3:7]=qpos[3:7]=[0.7071,0.7071,0,0]
    qpos[3:7]*=self.env.np_random.uniform(size=4,low=-1,high=1)
