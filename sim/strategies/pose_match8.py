...
class PoseMatch8:
  ...
  def init(self, total_timesteps=0):
    ...
    self.generate_reference_trajectory(self.supine_pose, self.script)

  def generate_reference_trajectory(self, starting_pose, script):
      """
      ...
      for i, keyframe in enumerate(script):
        ...
        duration_steps = keyframe['time']  # Assuming 'time' is in number of steps
        for step in range(duration_steps):
          ref_env.data.ctrl[:self.action_size] = current_ctrl
          mujoco.mj_step(ref_env.robot,ref_env.data,nstep=self.frames_per_action)
  ...
  def get_reference_motion(self, step_number):
    # Return the reference qpos and qvel for the current timestep.
    # If step_number exceeds ref_length, wrap around or clip.
    idx = min(step_number, self.ref_length-1)
    return self.ref_qpos[idx], self.ref_qvel[idx], self.ref_ctrl[idx]
  ...
  def step(self, action):
    ...
    ref_qpos, ref_qvel, _ = self.get_reference_motion(self.step_number)
  ...
  def apply_action(self, action):
    # Apply a low-pass filter to the action
    if not hasattr(self,'filtered_action'):
      self.filtered_action=np.zeros_like(action)

    alpha=0.3  # Low-pass filter coefficient, adjust as necessary
    self.filtered_action=alpha*action+(1-alpha)*self.filtered_action

    # Use the filtered action
    self.env.data.ctrl[:]=self.filtered_action
