...
from ergon_env_ul import ErgonEnvUl
import transforms3d
...
class Warmup1:
  ...
  def before_sim(self,action):
    ...
    self.env.target_data.qpos[:]=self.env.data.qpos[:]+np.random.uniform(-0.1,0.1,size=23)
    self.env.target_data.qvel[:]=self.env.data.qvel[:]+np.random.uniform(-0.1,0.1,size=22)
    # Target controls are the home controls
    self.env.target_data.ctrl[:]=self.home_ctrl
    # Start data at the target data
    self.env.data.qpos[:]=self.env.target_data.qpos[:]
    self.env.data.qvel[:]=self.env.target_data.qvel[:]
    # Step the target so we know where to go
    self.env.step_target()
  ...
  def step(self,action):
    ...
    # Penalize not matching the pose
    reward -= self.euclidean_distance(joint_pos[:3], self.env.target_data.qpos[:3])
    reward -= self.quaternion_distance(joint_pos[4:7], self.env.target_data.qpos[4:7])
    reward -= np.sum(self.euler_distance(joint_pos[7:], self.env.target_data.qpos[7:]))
