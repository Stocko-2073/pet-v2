...
class Balance26:
  ...
  def before_sim(self,action):
    ...
    self.curriculum_progress = self.curriculum_step / self.total_timesteps
  ...
  def sim_observation(self, value):
    return value * (1.0 - self.curriculum_progress)

  def real_observation(self, value):
    return value * self.curriculum_progress
  ...
  def observe(self):
    ...
    joint_pos=self.sim_observation(np.array(self.env.data.qpos))
