...
class Balance19:
  ...
  def _calculate_reward(self,joint_pos,chest_accel,torso_accel):
    ...
    orientation_penalty=-np.sum(self.parametric_gaussian(np.array([roll,pitch]),mean=0,sigma=1))*2
    ...
    # Gaussian penalty based on distance from origin
    position_penalty=-self.POSITION_WEIGHT*np.sum(self.parametric_gaussian(np.array([x,y]),mean=0,sigma=1)
    )
  ...
  def apply_action(self,action):
    self.total_steps+=1
    if (self.total_steps+1)%self.RAND_UPPER_BODY_AFTER==0:
      # Randomize upper body
      self.upper_body=self.env.np_random.uniform(size=8,low=-2.156,high=2.156
      )  # print(f"Randomizing upper body controls: {self.upper_body}")
    # Apply the action
