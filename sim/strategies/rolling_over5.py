...
class RollingOver5:
  ...
  def matches_pose(self,joint_pos,pose,sigma):
    dist=(self.quaternion_distance(joint_pos[3:7],pose[3:7])+
          np.sum(self.euler_distance(joint_pos[7:],pose[7:])))
  ...
  def step(self,action):
    ...
    match_pose_reward=self.matches_pose(joint_pos,self.prone_pose,sigma=0.2)
  ...
  def reset(self):
    print("Max Prone Orientation Reward:",self.max_match_pose_reward)
