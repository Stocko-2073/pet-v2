...
class FlywheelEnv:
    ...
    def __init__(self, port: str, baudrate: int = 2000000, max_episode_steps: int = 1000,
        ...
        self.observation_space_high = np.array([1, 1023, 1000, 15, 180, 1], dtype=np.float32)
    ...
    def _normalize_state(self, state: np.ndarray) -> np.ndarray:
        ...
        normalized = 2.0 * (state - self.observation_space_low) / (
                    self.observation_space_high - self.observation_space_low) - 1.0
        return np.clip(normalized, -1.0, 1.0)

    def _denormalize_state(self, normalized_state: np.ndarray) -> np.ndarray:
        """Convert normalized state back to original scale"""
        return self.observation_space_low + (normalized_state + 1.0) * (
                    self.observation_space_high - self.observation_space_low) / 2.0
    ...
    def get_oscillation_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'velocity_variance': np.var(list(self.position_velocity_history)) if len(
                self.position_velocity_history) > 1 else 0.0,
